...
const toLastNames = people => {
  ...
  }); // Did this w/ Ella and my npm test isn't working, but nothing has changed, and I did the later ones first before this one.
};
...
const reversedString = (str) => {
  ...
  const reversedString = array.reduce((rev, char) => {
    return char + rev;
  }, "");
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((totalKids, character) => {
    return character.children ? totalKids + character.children.length
    : totalKids;
  }, 0);
};
